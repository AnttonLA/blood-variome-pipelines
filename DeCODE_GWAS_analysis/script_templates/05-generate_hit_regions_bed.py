...
with open("variant_regions_step2.bed", "w") as f:
    subprocess.run(
        ["/home/antton/Programs/bedtools2/bin/bedtools", "merge", "-i", "variant_regions_step1.bed", "-header", "-c",
         "4", "-o", "distinct"],
        check=True, stdout=f)

# Read the collapsed bed file and convert it to a polars dataframe
collapsed_bed_df = pl.read_csv("variant_regions_step2.bed", sep="\t",
...
collapsed_bed_df.insert_at_idx(3,
                               collapsed_bed_df.select("name").apply(lambda x: len(str(x).split(',')) - 1).to_series())
